...
An interactive timeline of the **First World War** (1914–1918), drawn from
[Wikidata](https://www.wikidata.org) and rendered with the
[timel.in](https://github.com/openhistorymap/timelin) timeline core.

- Every Wikidata item that is *part of* WWI with a date — battles, operations,
  treaties, etc. — laid out as **swimlanes by type or by country**.
...
```bash
python3 scripts/build_events.py     # WW1 = Q361
```

The renderer (`src/app.ts`) and the timel.in core (`lib/core/`, vendored) are
shared verbatim with `timeline.ofww2.org`; only `src/main.ts`, `public/events.json`
